...
<nav class='h-24 flex justify-between bg-dark1 border-b-accent border-b-2 relative z-30 backdrop-blur font-semibold box-border'>
    <div class='w-1/4 flex items-center justify-center z-10 gap-4 max-md:w-1/2 max-md:px-12 max-md:justify-start' id='logo'>
    {#if $page.data.session?.user?.image ?? false}
        <!-- svelte-ignore a11y-img-redundant-alt -->
        <img src={$page.data.session?.user?.image} alt='profile picture' class='w-12 h-12 rounded-full max-md:w-16 max-md:h-16' />
    {:else}
    ...
    <div class={`w-3/4 flex justify-center z-20 max-md:h-screen max-md:bg-dark1 max-md:border-b-2 max-md:border-accent max-md:w-screen max-md:fixed max-md:flex-col max-md:transition-transform max-md:duration-500 max-md:ease-in-out ${$toggled === true ? 'max-md:translate-y-0' : 'max-md:-translate-y-full'}`} id='mobile'>
