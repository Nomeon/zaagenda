...
<div in:fade id='divider' class='rounded-lg relative flex flex-row overflow-hidden min-h-[6rem] w-11/12 bg-opacity-60 border-2 border-accent bg-dark1 backdrop-blur shadow-md shadow-accent'>
    ...
    <button on:click={() => deleteUserFromGroup(user._id)} class='absolute top-8 right-4 h-8 w-8 text-accent'>
        <MdDeleteForever />
    </button>
</div>
...
<style>
    #divider::before{ 
        ...
        /* background-position: 0% 50%;  background-image: url('data:image/svg+xml;charset=utf8, <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 77.25 381" preserveAspectRatio="none"><g fill="%230f172a"><path d="M8.47 0l33.86 381H0V0z"/><path d="M33.87 0l25.4 381H0V0z" opacity=".33"/><path d="M59.27 0l17.98 381H0V0z" opacity=".33"/></g></svg>');  */
